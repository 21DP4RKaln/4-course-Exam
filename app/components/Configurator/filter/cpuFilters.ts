...
export const createCpuFilterGroups = (
  ...
  if (speedSet.size) {
    const options: FilterOption[] = Array.from(speedSet).map(val => ({
      id: `speed=${val}`,
      name: val,
    }));
    groups.push({
  ...
  components.forEach(c => {
    const boost =
      c.specifications?.['Boost_Clock'] || c.specifications?.['boostClock'];
    if (boost) boostClockSet.add(String(boost));
  });
  if (boostClockSet.size) {
    const options: FilterOption[] = Array.from(boostClockSet).map(val => ({
      id: `boostClock=${val}`,
      name: val,
    }));
    groups.push({
  ...
  if (tdpSet.size) {
    const options: FilterOption[] = Array.from(tdpSet).map(val => ({
      id: `tdp=${val}`,
      name: val,
    }));
    groups.push({
  ...
  components.forEach(c => {
    const arch =
      c.specifications?.['architecture'] || c.specifications?.['Architecture'];
