...
export const createStorageFilterGroups = (
  ...
  components.forEach(c => {
    const capacity =
      c.specifications?.['Capacity'] ||
      c.specifications?.['capacity'] ||
      c.specifications?.['Storage'];
    if (capacity) {
 ...
 */
export const filterStorageComponents = (
  components: Component[],
  ...
  return components.filter(component => {
    for (const [filterId, isActive] of Object.entries(activeFilters)) {
      ...
      switch (filterType) {
        case 'capacity':
          const capacity =
            component.specifications?.['Capacity'] ||
            component.specifications?.['capacity'] ||
        ...
        case 'interface':
          ...
          if (interfaceType && String(interfaceType) !== filterValue)
            return false;
          break;
      }
    }
