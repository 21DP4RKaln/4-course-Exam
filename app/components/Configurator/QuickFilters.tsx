...
const QuickFilters: React.FC<Props> = ({ 
  ...
  const currentFilter = quickCpuFilter !== undefined ? quickCpuFilter : activeFilter
  const currentOnFilterChange = onQuickCpuFilterChange || onFilterChange
  
  const [lastActiveFilter, setLastActiveFilter] = useState<string | null>(currentFilter || null)
  const containerRef = useRef<HTMLDivElement>(null)
  const MAX_VISIBLE_ITEMS = 6
  const getBrandColor = useCallback((filterId: string | null, isActive: boolean) => {
    if (!isActive) {
      return theme === 'dark' ? 'text-neutral-400 hover:text-neutral-200 active:text-neutral-100' : 'text-neutral-600 hover:text-neutral-900 active:text-neutral-800'
    }
    
    if (!filterId) return theme === 'dark' ? 'text-red-400' : 'text-blue-600'
      // CPU brands and series
    if (filterId.toLowerCase().includes('intel') || filterId.toLowerCase().includes('core')) {
      return 'text-blue-500'
    }
    if (filterId.toLowerCase().includes('amd') || filterId.toLowerCase().includes('ryzen') || filterId.toLowerCase().includes('threadripper')) {
      return 'text-orange-500'
    }    // GPU brands and series
    if (filterId.toLowerCase().includes('nvidia') || filterId.toLowerCase().includes('rtx') || filterId.toLowerCase().includes('gtx')) {
      return 'text-green-500'
    }
    if (filterId.toLowerCase().includes('amd') && filterId.toLowerCase().includes('rx')) {
      return 'text-red-500'
    }
    if (filterId.toLowerCase().includes('intel') && (filterId.toLowerCase().includes('arc') || filterId.toLowerCase().includes('a580'))) {
    ...
    if (filterId.toLowerCase().includes('atx') && !filterId.toLowerCase().includes('micro') && !filterId.toLowerCase().includes('mini')) {
      return 'text-purple-500'
    }
    if (filterId.toLowerCase().includes('micro-atx') || filterId.toLowerCase().includes('microatx')) {
      return 'text-cyan-500'
    }
    if (filterId.toLowerCase().includes('mini-itx') || filterId.toLowerCase().includes('mini-atx') || filterId.toLowerCase().includes('miniitx')) {
    ...
    if (filterId.toLowerCase().includes('sata-ssd') || filterId.toLowerCase().includes('sata ssd')) {
      return 'text-orange-400'
    }
    if (filterId.toLowerCase().includes('hdd')) {
      return 'text-gray-500'
    }    // Cooling types
    if (filterId.toLowerCase().includes('air') && filterId.toLowerCase().includes('cooling')) {
      return 'text-sky-400'
    }
    if (filterId.toLowerCase().includes('liquid') && filterId.toLowerCase().includes('cooling')) {
      return 'text-blue-600'
    }
    if (filterId.toLowerCase().includes('fluid') && filterId.toLowerCase().includes('cooling')) {
      return 'text-blue-600'
    }
    // Services
    ...
    if (filterId.toLowerCase().includes('wifi') || filterId.toLowerCase().includes('bluetooth')) {
      return 'text-green-500'
    }
  ...
  const getBrandGlow = useCallback((filterId: string | null, isActive: boolean) => {
    if (!isActive) return ''
    
    if (!filterId) return theme === 'dark' ? 'shadow-[0_0_20px_rgba(239,68,68,0.5)]' : 'shadow-[0_0_20px_rgba(37,99,235,0.4)]'
      // CPU brands and series
    if (filterId.toLowerCase().includes('intel') || filterId.toLowerCase().includes('core')) {
    ...
    return theme === 'dark' ? 'shadow-[0_0_20px_rgba(239,68,68,0.5)]' : 'shadow-[0_0_20px_rgba(37,99,235,0.4)]'
  }, [theme])
  ...
  const getActiveFilterIndex = useCallback(() => {
    return allFilters.findIndex(filter => {
      ...
      return isQuickFilterActive && filter.id ? isQuickFilterActive(filter.id) : false;
    });
  ...
  }, [getActiveFilterIndex, currentIndex, maxIndex, MAX_VISIBLE_ITEMS, isUserScrolling]);  
  ...
  return (
    <div className={`relative h-[45px] ${
      theme === 'dark' 
    ...
    } rounded-lg flex items-center overflow-hidden transition-colors duration-200`} ref={containerRef}>      {canScrollLeft && (
        <button 
        ...
        {visibleFilters.map((filter, index) => {
          const isActive = filter.id === null 
            ? Object.keys(activeFilters).length === 0 // "All" is active when no filters are active
            : (isQuickFilterActive && filter.id ? isQuickFilterActive(filter.id) : false);
