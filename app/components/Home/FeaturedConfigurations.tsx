...
export default function FeaturedConfigurations() {
    ...
    const fetchConfigurations = async () => {
      ...
      try {
        const response = await fetch(
          '/api/configurations/popular?limit=4&sortBy=viewCount'
        );
        if (!response.ok) {
        ...
        const sortedConfigurations = Array.isArray(data)
          ? [...data].sort((a, b) => {
  ...
  const handleToggleWishlist = (
    ...
    if (!isAuthenticated) {
      router.push(
        `/${locale}/auth/login?returnUrl=${encodeURIComponent(pathname)}`
      );
      return;
    }
        ...
        {configurations.map((config, index) => (
          ...
          >
                  ...
                  configurations.findIndex(
                    c =>
                      ...
                      c.orderCount ===
                        Math.max(
                          ...configurations
                            .filter(conf => conf.orderCount)
                            .map(conf => conf.orderCount || 0)
                        )
            ...
            <div className="p-3 sm:p-4">
              <Link href={`/${locale}/shop/product/${config.id}`}>
                {' '}
                <h3 className="font-semibold text-sm md:text-base text-neutral-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-brand-red-400 transition-colors line-clamp-1">
                  {config.name}
                </h3>
              </Link>
