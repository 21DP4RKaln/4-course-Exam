...
export default function Footer() {
  ...
  const isCompanyInfoInView = useInView(companyInfoRef, {
    once: false,
    amount: 0.3,
  });
  const isQuickLinksInView = useInView(quickLinksRef, {
    once: false,
    amount: 0.3,
  });
  const isProductsInView = useInView(productsRef, { once: false, amount: 0.3 });
  const isOtherLinksInView = useInView(otherLinksRef, {
    once: false,
    amount: 0.3,
  });
              ...
              <motion.li variants={itemVariants}>
                ...
                >
                  ...
                  >
                    <Cpu
                      size={14}
                      className={`mr-2 ${theme === 'dark' ? 'text-brand-red-500' : 'text-brand-blue-500'}`}
                    />
          ...
          >
            ...
            <motion.ul className="space-y-3 mb-8" variants={containerVariants}>
              <motion.li className="flex items-start" variants={itemVariants}>
                <MapPin
                  size={16}
                  className={`mr-2 mt-0.5 flex-shrink-0 ${theme === 'dark' ? 'text-brand-red-500' : 'text-brand-blue-500'}`}
                />
              ...
              <motion.li className="flex items-center" variants={itemVariants}>
                <Phone
                  size={16}
                  className={`mr-2 flex-shrink-0 ${theme === 'dark' ? 'text-brand-red-500' : 'text-brand-blue-500'}`}
                />
            ...
            <motion.ul className="space-y-2" variants={containerVariants}>
              <motion.li className="flex items-start" variants={itemVariants}>
                <Clock
                  size={16}
                  className={`mr-2 mt-0.5 ${theme === 'dark' ? 'text-brand-red-500' : 'text-brand-blue-500'}`}
                />
