...
export default function ReadyConfigForm({ configId, isEditMode = false }: ReadyConfigFormProps) {
  ...
  useEffect(() => {
    ...
    const fetchConfigData = async () => {
      if (isEditMode && configId) {
        try {
          const response = await fetch(`/api/admin/ready-configs/${configId}`, {
            method: 'GET',
            cache: 'no-store'
          });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = isEditMode 
        ? `/api/admin/ready-configs/${configId}`
        : '/api/admin/ready-configs';
      ...
      router.push(`/${locale}/manage-ready-configs`);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link
          href={`/${locale}/manage-ready-configs`}
