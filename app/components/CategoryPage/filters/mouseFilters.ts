...
export const createMouseFilterGroups = (components: Component[]): FilterGroup[] => {
  ...
  const sensorOptions: FilterOption[] = Array.from(sensorSet).map(v => ({ id: `sensor=${v}`, name: v }))
  ...
  const connOptions: FilterOption[] = Array.from(connSet).map(v => ({ id: `connection=${v}`, name: v }))
