...
export const createKeyboardFilterGroups = (
  ...
  const switchOptions: FilterOption[] = Array.from(switchSet).map(v => ({
    id: `switch=${v}`,
    name: v,
  }));
  ...
  components.forEach(c => {
    const val =
      c.specifications['backlight'] ||
      c.specifications['lighting'] ||
      c.specifications['backlighting'];
    if (val) backlightSet.add(val);
  });
  const backlightOptions: FilterOption[] = Array.from(backlightSet).map(v => ({
    id: `backlight=${v}`,
    name: v,
  }));
