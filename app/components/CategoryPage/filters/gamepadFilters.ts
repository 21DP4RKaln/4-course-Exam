...
export const createGamepadFilterGroups = (components: Component[]): FilterGroup[] => {
  ...
  components.forEach(c => {
    const connection = c.gamepad?.connection || c.specifications?.['connection'] || c.specifications?.['connectivity']
    if (connection) connectionSet.add(connection)
  })
  const connectionOptions: FilterOption[] = Array.from(connectionSet).map(conn => ({
    id: `connection=${conn}`,
    name: conn
  }))
  ...
  components.forEach(c => {
    const platform = c.gamepad?.platform || c.specifications?.['platform'] || c.specifications?.['compatibility']
    if (platform) platformSet.add(platform)
  })
  const platformOptions: FilterOption[] = Array.from(platformSet).map(platform => ({
  ...
  components.forEach(c => {
    const layout = c.gamepad?.layout || c.specifications?.['layout'] || c.specifications?.['type']
  ...
  components.forEach(c => {
    const rgb = c.gamepad?.rgb || c.specifications?.['rgb'] || c.specifications?.['lighting']
  ...
  components.forEach(c => {
    const vibration = c.gamepad?.vibration || c.specifications?.['vibration'] || c.specifications?.['haptic']
    if (vibration !== undefined) vibrationSet.add(String(vibration))
  })
  const vibrationOptions: FilterOption[] = Array.from(vibrationSet).map(vib => ({
    id: `vibration=${vib}`,
  ...
  components.forEach(c => {
    const programmable = c.gamepad?.programmable || c.specifications?.['programmable'] || c.specifications?.['customizable']
    if (programmable !== undefined) programmableSet.add(String(programmable))
  })
  const programmableOptions: FilterOption[] = Array.from(programmableSet).map(prog => ({
  ...
  if (platformOptions.length > 0) {
    filterGroups.push({ title: 'Platform', type: 'platform', options: platformOptions })
  }

  if (layoutOptions.length > 0) {
    filterGroups.push({ title: 'Layout', type: 'layout', options: layoutOptions })
  }
  ...
  if (vibrationOptions.length > 0) {
    filterGroups.push({ title: 'Vibration', type: 'vibration', options: vibrationOptions })
  }

  if (programmableOptions.length > 0) {
    filterGroups.push({ title: 'Programmable', type: 'programmable', options: programmableOptions })
  }
