...
export const createMotherboardFilterGroups = (
  ...
  components.forEach(c => {
    const chipset =
      c.specifications?.['Chipset'] || c.specifications?.['chipset'];
    if (chipset) {
  ...
  components.forEach(c => {
    const ramSlots =
      c.specifications?.['RAM Slots'] ||
      c.specifications?.['ramSlots'] ||
      c.specifications?.['Memory Slots'];
    if (ramSlots) {
 ...
 */
export const filterMotherboardComponents = (
  components: Component[],
  ...
  return components.filter(component => {
    for (const [filterId, isActive] of Object.entries(activeFilters)) {
      ...
      switch (filterType) {
        ...
        case 'chipset':
          const chipset =
            component.specifications?.['Chipset'] ||
            component.specifications?.['chipset'];
          if (chipset && String(chipset) !== filterValue) return false;
          break;
        case 'ramSlots':
          const ramSlots =
            component.specifications?.['RAM Slots'] ||
            component.specifications?.['ramSlots'] ||
            component.specifications?.['Memory Slots'];
