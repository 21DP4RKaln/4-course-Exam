...
export const createMicrophoneFilterGroups = (components: Component[]): FilterGroup[] => {
  ...
  components.forEach(c => {
    const type = c.specifications['type'] || c.specifications['micType'] || c.specifications['microphoneType']
  ...
  components.forEach(c => {
    const pattern = c.specifications['polarPattern'] || c.specifications['pattern'] || c.specifications['directivity']
    if (pattern) patternSet.add(pattern)
  })
  const patternOptions: FilterOption[] = Array.from(patternSet).map(v => ({ id: `pattern=${v}`, name: v }))
  ...
  components.forEach(c => {
    const usage = c.specifications['usage'] || c.specifications['application'] || c.specifications['purpose']
    if (usage) usageSet.add(usage)
  })
  const usageOptions: FilterOption[] = Array.from(usageSet).map(v => ({ id: `usage=${v}`, name: v }))

  // Phantom power requirement options
  const phantomSet = new Set<string>()
  components.forEach(c => {
    const phantom = c.specifications['phantomPower'] || c.specifications['powerRequirement'] || c.specifications['power']
    if (phantom) phantomSet.add(phantom)
  })
  const phantomOptions: FilterOption[] = Array.from(phantomSet).map(v => ({ id: `phantom=${v}`, name: v }))
  ...
  if (typeOptions.length > 0) {
    filterGroups.push({ title: 'Microphone Type', type: 'type', options: typeOptions })
  }

  if (patternOptions.length > 0) {
    filterGroups.push({ title: 'Polar Pattern', type: 'pattern', options: patternOptions })
  }
  ...
  if (usageOptions.length > 0) {
    filterGroups.push({ title: 'Usage Type', type: 'usage', options: usageOptions })
  }

  if (phantomOptions.length > 0) {
    filterGroups.push({ title: 'Phantom Power', type: 'phantom', options: phantomOptions })
  }
