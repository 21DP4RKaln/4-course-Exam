...
export const createSpeakerFilterGroups = (
  ...
  components.forEach(c => {
    const type =
      ...
      c.specifications['speakerType'] ||
      c.specifications['configuration'];
  ...
  components.forEach(c => {
    const power =
      ...
      c.specifications['maxPower'];
    if (power) powerSet.add(power);
  });
  const powerOptions: FilterOption[] = Array.from(powerSet).map(v => ({
    id: `power=${v}`,
    name: v,
  }));
  ...
  components.forEach(c => {
    const driver =
      c.specifications['driverSize'] ||
      c.specifications['driver'] ||
      c.specifications['wooferSize'];
    if (driver) driverSet.add(driver);
  });
  const driverOptions: FilterOption[] = Array.from(driverSet).map(v => ({
    id: `driver=${v}`,
    name: v,
  }));
  ...
  components.forEach(c => {
    const wireless =
      c.specifications['wireless'] ||
      c.specifications['bluetooth'] ||
      c.specifications['wifi'];
    if (wireless) wirelessSet.add(wireless);
  });
  const wirelessOptions: FilterOption[] = Array.from(wirelessSet).map(v => ({
    id: `wireless=${v}`,
    name: v,
  }));
