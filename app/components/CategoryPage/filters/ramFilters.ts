...
export const createRamFilterGroups = (
  ...
  components.forEach(c => {
    const voltage =
      c.specifications?.['Voltage'] || c.specifications?.['voltage'];
  ...
  components.forEach(c => {
    ...
    const hasHeatspreader =
      ...
      c.specifications?.['Heatspreader'] ||
      c.name.toLowerCase().includes('heatspreader') ||
