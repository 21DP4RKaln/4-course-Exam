...
export default function ProfileTab() {
  ...
  const validateForm = () => {
    ...
    if (!email && !phone) {
      setError('You must provide either an email address or phone number')
      return false
    }
        ...
        <div className="p-4 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 rounded-md flex items-start mb-4">
          ...
          <div>
            ...
            <p className="text-sm mt-1">
              Please add {!user?.email ? 'an email address' : ''}{!user?.email && !user?.phone ? ' and ' : ''}{!user?.phone ? 'a phone number' : ''} to your profile.
            </p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="bg-gray-50 dark:bg-gray-900 p-6 rounded-lg">
        ...
        <div className="mt-6 flex justify-end space-x-4">
          ...
          <button
            ...
            className="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 flex items-center disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
