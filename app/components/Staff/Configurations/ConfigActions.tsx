...
export function ConfigActions({
  ...
  const handleView = () => {
    if (onView) {
      ...
      router.push(`/${user?.role.toLowerCase()}/configurations/${configuration.id}`)
    }
  }
  ...
  const handleEdit = () => {
    if (onEdit) {
      ...
      router.push(`/${user?.role.toLowerCase()}/configurations/${configuration.id}/edit`)
    }
  }
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this configuration?')) {
      if (onDelete) {
        ...
        try {
          const response = await fetch(`/api/staff/configurations/${configuration.id}`, {
            method: 'DELETE'
          })
          if (response.ok) {
            router.refresh()
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: onPublish || (() => router.push(`/${user?.role.toLowerCase()}/configurations/${configuration.id}/publish`)),
      show: (isAdmin || isSpecialist) && configuration.status === 'APPROVED' && !configuration.isPublic,
