...
export function ConfigList({
  ...
  const [publishingConfig, setPublishingConfig] =
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'DRAFT':
        return (
          <span className="px-2 py-1 text-xs rounded-full bg-neutral-100 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-300 flex items-center gap-1">
            <Clock size={12} /> Draft
          </span>
        );
      case 'SUBMITTED':
        return (
          <span className="px-2 py-1 text-xs rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 flex items-center gap-1">
            <Send size={12} /> Pending Review
          </span>
        );
      case 'APPROVED':
        return (
          <span className="px-2 py-1 text-xs rounded-full bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 flex items-center gap-1">
            <CheckCircle size={12} /> Approved
          </span>
        );
      case 'REJECTED':
        return (
          <span className="px-2 py-1 text-xs rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 flex items-center gap-1">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      {config.status === 'SUBMITTED' && (
                        <>
                          <button
                            onClick={() =>
                              handleStatusUpdate(config.id, 'APPROVED')
                            }
                          ...
                          <button
                            onClick={() =>
                              handleStatusUpdate(config.id, 'REJECTED')
                            }
                      ...
                      <button
                        onClick={() =>
                          router.push(
                            `/${user?.role.toLowerCase()}/configurations/${config.id}/edit`
                          )
                        }
