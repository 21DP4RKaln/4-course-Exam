  ...
  const columns: Column<Component>[] = [
    ...
    },
    {
      ...
      render: (value: number | null, row: Component) => {
        if (!value) return '-';
        const isExpired =
          row.discountExpiresAt && new Date(row.discountExpiresAt) < new Date();
        if (isExpired) return '-';
      ...
      render: (item: Component) => {
        ...
        return (
          <ActionButtons
            onView={() => {
              console.log(
                `Navigating to view: /${locale}/admin/components/${item.id}`
              );
              router.push(`/${locale}/admin/components/${item.id}`);
            }}
            onEdit={() => {
              console.log(
                `Navigating to edit: /${locale}/admin/components/${item.id}/edit`
              );
