...
export default function PeripheralsPage() {
  ...
  const filteredPeripherals = peripherals.filter(peripheral => {
    if (selectedCategory && peripheral.category !== selectedCategory) return false
    if (searchQuery && !peripheral.name.toLowerCase().includes(searchQuery.toLowerCase())) return false
    return true
  })
              ...
              <tbody>
                {filteredPeripherals.map((peripheral) => (
                  <tr key={peripheral.id} className="border-b">
                    ...
                    <td className="px-6 py-4 capitalize">{peripheral.category}</td>
                    <td className="px-6 py-4">{peripheral.sku}</td>
                    <td className="px-6 py-4 text-right">€{peripheral.price}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs ${
                        peripheral.stock > 10 ? 'bg-green-100 text-green-800' :
                        peripheral.stock > 0 ? 'bg-yellow-100 text-yellow-800' :
