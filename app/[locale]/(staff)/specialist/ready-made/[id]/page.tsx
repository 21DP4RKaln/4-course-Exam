...
export default function ReadyMadePCDetailsPage() {
  ...
  const fetchPCDetails = async () => {
    try {
      const response = await fetch(`/api/specialist/products/ready-made/${params.id}`)
      if (response.ok) {
  ...
  const handlePublish = async () => {
    try {
      const response = await fetch(`/api/specialist/products/ready-made/${params.id}/publish`, {
        method: 'POST'
      })
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Badge className={pc.isPublic ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                  {pc.isPublic ? t('shop.visibility.public') : t('shop.visibility.private')}
                </Badge>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid gap-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <h4 className="font-medium mb-1">{t('shop.table.category')}</h4>
                  <p className="capitalize">{t(`shop.filters.${pc.category.toLowerCase()}`)}</p>
                </div>
                <div>
                  <h4 className="font-medium mb-1">{t('common.createdDate')}</h4>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b bg-gray-50 dark:bg-gray-800">
                        <th className="px-4 py-2 text-left">{t('shop.table.component')}</th>
                        <th className="px-4 py-2 text-left">{t('shop.table.category')}</th>
                        <th className="px-4 py-2 text-center">{t('shop.table.quantity')}</th>
                        <th className="px-4 py-2 text-right">{t('shop.table.price')}</th>
                      </tr>
                    </thead>
                    <tbody>
