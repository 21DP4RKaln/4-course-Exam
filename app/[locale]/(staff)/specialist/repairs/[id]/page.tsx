...
export default function RepairDetailsPage() {
  ...
  const saveChanges = async () => {
    try {
      ...
      if (
        estimatedCost &&
        parseFloat(estimatedCost) !== repair?.estimatedCost
      ) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={e =>
                      setSelectedStatus(e.target.value as RepairStatus)
                    }
                    disabled={
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={e =>
                      setSelectedPriority(e.target.value as RepairPriority)
                    }
                    disabled={
                      repair.status === 'COMPLETED' ||
                      repair.status === 'CANCELLED'
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          {repair.product.type === 'configuration' &&
                            repair.product.components && (
                              <details className="mt-2">
                                <summary className="cursor-pointer text-sm text-blue-600 dark:text-blue-400">
                                  Show Components (
                                  {repair.product.components.length})
                                </summary>
            ...
            <CardFooter className="flex flex-wrap gap-3 justify-between">
              <div className="flex gap-3">
                ...
                <Button
                  onClick={() =>
                    router.push(
                      `/${locale}/specialist/repairs/${repair.id}/parts`
                    )
                  }
                  disabled={
                    repair.status === 'COMPLETED' ||
                    ...
                    {repair.specialists.map(specialist => (
                      <div key={specialist.id} className="text-sm">
                        ...
                        <div className="text-xs">
                          Assigned:{' '}
                          {new Date(specialist.assignedAt).toLocaleDateString()}
                        </div>
                      </div>
                    ))}
                  </div>
