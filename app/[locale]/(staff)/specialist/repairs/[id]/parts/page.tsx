                  ...
                  <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                    <thead>
                      <tr>
                        ...
                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">Total</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                      {existingParts.map(part => (
                        <tr key={part.id}>
                          ...
                          <td className="px-4 py-3 text-right">€{(part.price * part.quantity).toFixed(2)}</td>
                        </tr>
                      ))}
                      <tr className="font-medium">
                        <td colSpan={4} className="px-4 py-3 text-right">Total:</td>
                        <td className="px-4 py-3 text-right">
                          €{existingParts.reduce((total, part) => total + (part.price * part.quantity), 0).toFixed(2)}
                        </td>
                      </tr>
                    </tbody>
                  </table>
          ...
          <Card>
                  ...
                  >
                    ...
                    {components.map(component => (
                      <option key={component.id} value={component.id}>
                        {component.name} ({component.category}) - €{component.price}
                      </option>
                    ))}
                  </select>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                      ...
                      <tbody className="divide-y divide-neutral-200 dark:divide-neutral-800">
                        {partsToAdd.map((part, index) => (
                          <tr key={index}>
                            <td className="px-4 py-3 whitespace-nowrap">{getComponentName(part.componentId)}</td>
                            <td className="px-4 py-3 whitespace-nowrap">{getComponentCategory(part.componentId)}</td>
                            <td className="px-4 py-3 text-center">{part.quantity}</td>
                            <td className="px-4 py-3 text-right">€{part.price.toFixed(2)}</td>
                            <td className="px-4 py-3 text-right">€{(part.price * part.quantity).toFixed(2)}</td>
                        ...
                        <tr className="font-medium">
                          <td colSpan={4} className="px-4 py-3 text-right">Total to add:</td>
                          <td className="px-4 py-3 text-right">
                            €{partsToAdd.reduce((total, part) => total + (part.price * part.quantity), 0).toFixed(2)}
                          </td>
                          <td></td>
                        </tr>
                      </tbody>
            ...
            <CardFooter>
              <Button 
                ...
                className="w-full"
              >
                {saving ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <Save className="h-4 w-4 mr-2" />}
                Save Parts
              </Button>
            </CardFooter>
          </Card>
            ...
            <CardContent className="text-sm space-y-4">
              ...
              <div className="space-y-2">
                ...
                <ol className="list-decimal pl-5 space-y-1">
                  ...
                  <li>Adjust the price if needed (default is the standard price)</li>
                  <li>Click "Add to List" to add it to your basket</li>
                  <li>When finished, click "Save Parts" to add them to the repair</li>
                </ol>
              </div>
              
              <p className="text-neutral-500 italic">
                Note: Adding parts will not affect inventory levels until the repair is completed.
              </p>
            </CardContent>
          </Card>
        </div>
