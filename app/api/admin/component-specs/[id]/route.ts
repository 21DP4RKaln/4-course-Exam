...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ 
      error: 'ComponentSpec model is not available in the current database schema',
      missingModels: ['ComponentSpec', 'SpecificationKey'],
      suggestedFix: 'Add these models to your schema.prisma file and run prisma migrate'
    }, { status: 404 });
  } catch (error) {
