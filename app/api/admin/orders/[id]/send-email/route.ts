...
import { createUnauthorizedResponse, createServerErrorResponse, createBadRequestResponse } from '@/lib/apiErrors'
...
export async function POST(
  ...
  try {
    ...
    if (!emailType || !['confirmation', 'approval'].includes(emailType)) {
      return createBadRequestResponse('Valid emailType required (confirmation or approval)')
    }
    ...
    try {
      ...
      await prisma.auditLog.create({
        data: {
          action: 'EMAIL_SENT',
          entityType: 'ORDER',
          entityId: params.id,          details: JSON.stringify({
            emailType,
            locale: orderLocale,
