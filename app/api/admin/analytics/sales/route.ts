...
export async function GET(request: NextRequest) {
  try {
    ...
    const [orders, totalRevenue, totalOrders, averageOrderValue, productMap] =
      await prisma.$transaction(async tx => {
        ...
        const totalRev = orderData.reduce(
    ...
    if (period === 'day') {
      ...
      salesData = Array.from(hourlyMap.values()).sort((a, b) => {
      ...
      salesData = Array.from(dailyMap.values()).sort((a, b) =>
      ...
      salesData = Array.from(monthlyMap.values()).sort((a, b) =>
        a.date.localeCompare(b.date)
      );
    }
