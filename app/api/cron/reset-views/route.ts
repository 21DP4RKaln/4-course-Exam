...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${request.nextUrl.origin}/api/shop/product/view?apiKey=${apiKey}`,
      {
        method: 'PUT',
      }
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to reset view counts: ${error.error || 'Unknown error'}`
      );
    }
    ...
    console.log(
      `[CRON] Monthly view counts reset - ${new Date().toISOString()}:`,
      result.resetCounts
    );
