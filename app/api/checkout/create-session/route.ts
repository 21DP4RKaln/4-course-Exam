...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `Stripe session created: ${stripeSession.id} for order: ${order.id}`
    );
    ...
    if (order && order.id) {
      try {
        ...
        console.log(
          `Order ${order.id} marked as cancelled due to Stripe session failure`
        );
      } catch (updateError) {
        console.error(
          'Failed to update order status after Stripe error:',
          updateError
        );
      }
    }
