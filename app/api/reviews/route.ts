...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedReviews = reviews.map(review => {
      ...
      let userVote = null;
      if (currentUserId) {
        const userVoteData = review.helpful.find(
          h => h.userId === currentUserId
        );
        if (userVoteData) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { productId, productType, rating, comment, purchaseDate } =
    ...
    const hasPurchased = await checkIfUserPurchased(
      userId,
      productId,
      productType
    );
...
async function checkIfUserPurchased(
  userId: string,
  productId: string,
  productType: string
): Promise<boolean> {
