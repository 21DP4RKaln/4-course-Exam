...
export async function seedCooling(prisma: PrismaClient) {
  ...
  for (let i = 0; i < coolingComponents.length; i++) {
    ...
    const coolingType =
      specs.coolingType || coolingTypes[i % coolingTypes.length];
    ...
    if (manufacturer === 'Noctua' && coolingType === 'Air') {
      ...
      model =
        coolingType === 'Air'
          ? ['Freezer 34 eSports DUO', 'Freezer 7X', 'Alpine 12', 'Freezer 50'][
              i % 4
            ]
          : [
    ...
    const socketCompatibility =
      specs.socketCompatibility || sockets[i % sockets.length];
    ...
    const fanRPM =
      specs.fanRPM || `${800 + (i % 12) * 200}-${1500 + (i % 15) * 200} RPM`;
    ...
    if (coolingType.includes('Liquid')) {
      radiatorSize =
        specs.radiatorSize || radiatorSizes[i % radiatorSizes.length];
    }
    coolingItems.push({
      ...
      radiatorSize: radiatorSize
        ? parseInt(radiatorSize.replace(/[^\d]/g, ''))
        : null,
      fanSize: fanSize,
  ...
  for (const cooling of additionalCooling) {
    ...
    const sizeLabel = isAir
      ? `${cooling.height}mm Height`
      : `${cooling.radiatorSize || 0}mm Radiator`;
    ...
    const price = Math.round(
      basePrice * performanceMultiplier * typeMultiplier * rgbMultiplier
    );
