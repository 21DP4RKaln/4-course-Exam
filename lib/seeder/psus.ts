...
export async function seedPSUs(prisma: PrismaClient) {
  ...
  for (let i = 0; i < psuComponents.length; i++) {
    ...
    const manufacturer = (specs.manufacturer ||
      manufacturers[i % manufacturers.length]) as ManufacturerType;
    ...
    const certification =
      specs.certification || certifications[i % certifications.length];
  ...
  for (const psu of additionalPSUs) {
    ...
    const efficiencyMultiplier = psu.efficiency.includes('Titanium')
      ? 1.8
      : psu.efficiency.includes('Platinum')
        ? 1.5
        : psu.efficiency.includes('Gold')
          ? 1.2
          : psu.efficiency.includes('Silver')
            ? 1.1
            : 1.0;

    const modularMultiplier =
      psu.modular === 'Full' ? 1.2 : psu.modular === 'Semi' ? 1.1 : 1.0;

    const price = Math.round(
      basePrice * wattageMultiplier * efficiencyMultiplier * modularMultiplier
    );
