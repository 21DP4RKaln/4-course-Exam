...
export async function seedCameras(prisma: PrismaClient) {
  ...
  for (let i = 0; i < cameraPeripherals.length; i++) {
    ...
    const resolution =
      specs.resolution || resolutions[Math.min(i, resolutions.length - 1)];
    ...
    const additionalSpecs = {
      ...
      lowLightPerformance:
        i % 4 === 0 ? 'Excellent' : i % 4 === 1 ? 'Good' : 'Average',
      dimensions: `${70 + (i % 5) * 10}mm x ${30 + (i % 5)}mm x ${30 + (i % 5)}mm`,
      microphoneType: hasMicrophone
        ? i % 2 === 0
          ? 'Stereo'
          : 'Mono'
        : 'None',
 ...
 */
async function addMoreCameras(prisma: PrismaClient) {
