...
export async function seedSpeakers(prisma: PrismaClient) {
  ...
  for (let i = 0; i < speakerPeripherals.length; i++) {
    ...
    // Types of speakers
    const types = ['2.0 Stereo', '2.1 Stereo', '5.1 Surround', '7.1 Surround', '2.0 Stereo'];
    ...
    const totalWattage = specs.totalWattage ? parseInt(specs.totalWattage) : wattageRanges[i % wattageRanges.length];
    ...
    const frequency = specs.frequency || specs.frequencyResponse || frequencyResponses[i % frequencyResponses.length];
    ...
    const connections = specs.connections || specs.connection || connectionTypes[i % connectionTypes.length];
    
    // Has bluetooth?
    const hasBluetooth = connections.includes('Bluetooth');
    
    // For surround systems
    const hasSubwoofer = type.includes('2.1') || type.includes('5.1') || type.includes('7.1');
    
    // Additional specifications that don't match the schema
    const additionalSpecs = {
      hasSubwoofer,
      hasTweeter: i % 3 === 0,
      controls: ['Volume, Bass', 'Volume, Treble, Bass', 'Volume', 'Full mixer'][i % 4],
