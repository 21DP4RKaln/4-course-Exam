...
export async function seedSpecificationKeys(prisma: PrismaClient) {
  ...
  const motherboardCategory = componentCategories.find(
    c => c.slug === 'motherboard'
  );
  ...
  const mousePadCategory = peripheralCategories.find(
    c => c.slug === 'mousepad'
  );
  const microphoneCategory = peripheralCategories.find(
    c => c.slug === 'microphone'
  );
  ...
  const headphonesCategory = peripheralCategories.find(
    c => c.slug === 'headphones'
  );
  const speakersCategory = peripheralCategories.find(
    c => c.slug === 'speakers'
  );
  ...
  const gpuSpecKeys = [
    {
      name: 'manufacturer',
      displayName: 'Manufacturer',
      componentCategoryId: gpuCategory?.id,
    },
    {
      name: 'chipset',
      displayName: 'Chipset',
  ...
  const keyboardSpecKeys = [
    {
      ...
      peripheralCategoryId: keyboardCategory?.id,
    },
    {
      name: 'layout',
      displayName: 'Layout',
    ...
    {
      name: 'multimedia_keys',
      displayName: 'Multimedia Keys',
      peripheralCategoryId: keyboardCategory?.id,
    },
  ];
