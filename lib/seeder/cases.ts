...
export async function seedCases(prisma: PrismaClient) {
  ...
  for (let i = 0; i < caseComponents.length; i++) {
    ...
    const validManufacturer = Object.keys(models).includes(manufacturer)
      ? manufacturer
      : 'Corsair';
    const model =
      specs.model ||
      models[validManufacturer][i % models[validManufacturer].length];
    ...
    cases.push({
      ...
      usbPorts:
        `${(i % 2) + 1}x USB 3.0, ${i % 2}x USB 2.0, ${i % 2 === 0 ? '1x USB-C' : ''}`.trim(),
      colorVariant: color,
    });
  }
  ...
  for (const caseItem of additionalCases) {
    ...
    const formFactorMultiplier =
      caseItem.formFactor === 'ATX Full Tower'
        ? 1.5
        : caseItem.formFactor === 'ATX Mid Tower'
          ? 1.2
          : caseItem.formFactor === 'Mini-ITX'
            ? 1.1
            : 1.0;

    const materialMultiplier = caseItem.materials.includes('Aluminum')
      ? 1.3
      : 1.0;
    ...
    const price = Math.round(
      basePrice * formFactorMultiplier * materialMultiplier * fansMultiplier
    );
