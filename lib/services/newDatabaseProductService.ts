...
export async function getProductById(
  ...
  try {
    ...
    if (type === 'component') {
      ...
      console.log(
        'Peripheral search result:',
        peripheral ? 'Found' : 'Not found'
      );
...
async function getProductRatings(
  ...
  try {
    ...
    console.error(
      `Error fetching ratings for ${productType} ${productId}:`,
      error
    );
    return { average: 0, count: 0 };
  }
...
export async function incrementProductView(
  ...
  try {
    ...
    console.error(
      `Failed to increment view count for ${productType} ${productId}:`,
      error
    );
  }
}
