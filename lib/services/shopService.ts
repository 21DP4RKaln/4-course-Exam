...
export async function getReadyMadePCs(): Promise<PC[]> {
  try {
    ...
    return configurations.map((config) => {
      ...
      const discountPrice = config.isPublic ? config.totalPrice * 0.9 : null;
...
export async function getPCById(id: string): Promise<PCWithRelated | null> {
  try {
    ...
    const discountPrice = configuration.isPublic ? configuration.totalPrice * 0.9 : null;
    ...
    const relatedPCs = relatedConfigurations.map((config) => {
      ...
      return {
        ...
        specs: relatedSpecs,
        price: config.totalPrice,
        discountPrice: config.isPublic ? config.totalPrice * 0.9 : null,
        imageUrl: null,
        featured: config.isPublic,
        stock: 10,
        ratings: {
