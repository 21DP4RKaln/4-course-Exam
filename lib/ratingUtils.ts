 ...
 */
export async function updateProductRating(productId: string, productType: ProductType): Promise<void> {
  try {
    ...
    console.error(`Error updating rating for ${productType} ${productId}:`, error)
    throw error
  }
}
 ...
 */
export async function updateAllProductRatings(productType: ProductType): Promise<void> {
  try {
    ...
    switch (productType) {
      ...
      default:
        throw new Error(`Unsupported product type for batch update: ${productType}`)
    }
    ...
    console.log(`Updated ratings for ${products.length} ${productType.toLowerCase()}s`)
  } catch (error) {
